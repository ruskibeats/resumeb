...
async function editRole(page, { editUrl, title, description, key }) {
  ...
  if (title) {
    const titleCandidates = [
      page.getByRole('textbox', { name: 'Ex: Retail Sales Manager' }).first(),
      page.getByRole('textbox', { name: /Title\*/i }).first(),
      page.locator('input[name="title"], input[id*="title" i], input[aria-label*="Title" i]').first(),
    ...
    if (!titleBox) throw new Error(`Could not find Title textbox for ${key}.`);
    await titleBox.fill('');
...
async function main() {
  ...
  try {
    ...
    if (SETUP_MODE) {
      console.log('Setup complete. Session already logged in:', USER_DATA_DIR);
      return;
    }
    ...
    // keep browser open for quick verification
  }
}
